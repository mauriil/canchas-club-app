...
import { Pie, PieChart, ResponsiveContainer, Sector, AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip } from "recharts";
    ...
    return (
        ...
        >
            <Paper elevation={3} sx={{
                ...
                <Typography variant="h1" sx={{ marginTop: { sm: props.loadingUserStats ? 0 : '40px', xs: props.loadingUserStats ? 0 : '15px' } }}>{
                    props.loadingUserStats && props.userStats.mostVisitedClubs !== undefined ?
                        <LineWave
                            height="100"
                            width="100"
                            color="#32d9cb"
                            ...
                            <PieChart width={300} height={300}>
                                <Pie
                                    ...
                                    activeShape={({
                                        ...
                                        return (
                                            <g>
                                                <text x={cx} y={cy} dy={8} fontSize={13} textAnchor="middle" fill={fill}>
                                                    {payload.name}
                                                </text>
                                                <Sector
                                                    cx={cx}
                                                ...
                                                <text x={ex} y={ey} fontSize={15} textAnchor={textAnchor} fill="#333">{`${(percent * 100).toFixed(2)}%`}</text>
                                            </g>
                                        );
                                    }}
                                    data={props.userStats.chartData.mostVisitedClubs.map((club: any) => {
                                        return {
                                            name: club.clubName,
                                            value: club.count
                                        }
                                    ...
                                    activeShape={({
                                        ...
                                        return (
                                            <g>
                                                ...
                                                <path d={`M${sx},${sy}L${mx},${my}L${ex},${ey}`} stroke={fill} fill="none" />
                                                <circle cx={ex} cy={ey} r={2} fill={fill} stroke="none" />
                                                <text x={ex + (cos >= 0 ? 1 : -1) * 12} y={ey} fontSize={15} textAnchor={textAnchor} fill="#333">{`${(percent * 100).toFixed(2)}%`}</text>
                                            </g>
                                        );
                                    }}
                                    data={props.userStats.chartData.mostVisitedFields.map((field: any) => {
                                        return {
                    ...
                    props.loadingUserStats ?
                        ...
                        <ResponsiveContainer width={380} height={300}>
                            <AreaChart
                                width={150}
                                height={300}
                                data={props.userStats.chartData.mostVisitedHours.map((hourData: any) => {
                                    return {
                                        name: hourData._id,
                            ...
                            >
                                ...
                                <YAxis fontSize={20} />
                                <Tooltip />
                                <Area type="monotone" dataKey="Reservas" stroke="#A9C52F" fill="#00a6c0" />
                            </AreaChart>
                        </ResponsiveContainer>
                }</Typography>
            </Paper>
